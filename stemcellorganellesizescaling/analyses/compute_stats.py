...
# Relative
from stemcellorganellesizescaling.analyses.utils.pairwise_stats_and_regression import (
    fit_ols,
    calculate_pairwisestats,
    explain_var_compositemodels,
    bootstrap_linear_and_log_model,
)

importlib.reload(
    sys.modules[
        "stemcellorganellesizescaling.analyses.utils.pairwise_stats_and_regression"
    ]
)
from stemcellorganellesizescaling.analyses.utils.pairwise_stats_and_regression import (
    fit_ols,
...
def compensate(
    ...
    for xi, xlabel in enumerate(
        ["cell_metrics_AVH", "cell_metrics_AV", "cell_metrics_H"]
    ):
        features_4_comp = FS[xlabel]
        ...
        for yi, ylabel in enumerate(FS["struct_metrics"]):
            ...
            for si, struct in enumerate(selected_structures):
                ...
                for ti, type in enumerate(["Linear", "Complex"]):
                    ...
                    yr = np.expand_dims(fittedmodel.resid, axis=1)
                    cells_COMP.loc[
                        cells_COMP["structure_name"] == struct,
                        f"{ylabel}_COMP_{type}_{xlabel}",
                    ] = yr
...
def pairwisestats(
    ...
    # %% Part 0 pairwise stats cell and nucleus measurement and PCA for new figure
    print("Cell and PCA metrics")
    D = {}
    for xi, xlabel in enumerate(FS["cell_pca_components"]):
        for yi, ylabel in enumerate(FS["cell_pca_components"]):
            if xlabel is not ylabel:
                ...
                D.update(
                    calculate_pairwisestats(
                        x, y, xlabel, ylabel, "None", SubSample_flag
                    )
                )
    # prepare directory
    save_dir = data_root / statsOUTdir / "cell_pca_metrics"
    ...
    for key in tqdm(D, "saving all computed statistics"):
        ...
        i += 1
    manifest.to_csv(save_dir / "cell_pca_metrics_manifest.csv")
    ...
    #             print(f"{struct}")
    #             x = (
    #                 cells.loc[cells["structure_name"] == struct, xlabel]
    #                 .squeeze()
    #                 .to_numpy()
    #             )
    #             x = np.expand_dims(x, axis=1)
    ...
    #             D.update(
    #                 calculate_pairwisestats(
    #                     x, y, xlabel, ylabel, struct, SubSample_flag
    #                 )
    #             )
    # # prepare directory
    ...
    #                             x = (
    #                                 cells_COMP.loc[
    #                                     cells_COMP["structure_name"] == struct, col2
    #                                 ]
    #                                 .squeeze()
    #                                 .to_numpy()
    #                             )
    ...
    #                             y = (
    #                                 cells_COMP.loc[
    #                                     cells_COMP["structure_name"] == struct, col1
    #                                 ]
    #                                 .squeeze()
    #                                 .to_numpy()
    #                             )
    ...
    #                             D.update(
    #                                 calculate_pairwisestats(
    #                                     x, y, col2, col1, struct, SubSample_flag
    #                                 )
    #                             )
    #     # prepare directory
...
# %% function defintion of regression model compensation
def compositemodels_explainedvariance(dirs: list, dataset: Path, statsOUTdir: Path):
    """
...
def scaling_stats(
    ...
    for yi, ylabel in enumerate(FS["cellnuc_metrics"]):
        ...
        y = cells[ylabel].squeeze().to_numpy()
        scaling_stats = bootstrap_linear_and_log_model(
            x, y, "Cell volume", ylabel, Type, cell_doubling, "None", Nbootstrap=100
        )
        ...
        for j, vol in enumerate(cell_doubling_interval):
            pos = (
                np.argwhere(
                    np.all(
                        np.concatenate(
                            (x2 >= (vol - shift), x2 <= (vol + shift)), axis=1
                        ),
                        axis=1,
                    )
                )
            ...
            if pos.size < 1:
                ...
                y_res[j, 2 : (len(perc_values) + 2)] = np.percentile(
                    y[pos], perc_values
                )
    ...
    for yi, ylabel in enumerate(FS["struct_metrics"]):
        ...
        for si, struct in enumerate(selected_structures):
            ...
            scaling_stats = bootstrap_linear_and_log_model(
                x, y, "Cell volume", ylabel, Type, cell_doubling, struct, Nbootstrap=100
            )
            ...
            for perc in np.arange(len(perc_values)):
                ScaleCurve[f"{ylabel}_{struct}_{perc_values[perc]}"] = y_res[
                    :, perc + 2
                ]
