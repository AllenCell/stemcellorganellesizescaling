...
# %%
def growplot(
    axGrow, metric1, metric2, ScaleCurve, structures, fs, stats_root,
):
    ...
    f = interpolate.interp1d(
        xii[:, 0], pred_yL, kind="linear", fill_value="extrapolate", bounds_error=False
    )
    y0 = f(cd0)
    y1 = f(cd1)
    ...
    axGrow.text(
        0.05, 0.9, f"{int(np.round(100*yPline[1]))}%", fontsize=fs, color=darkgreen
    )
