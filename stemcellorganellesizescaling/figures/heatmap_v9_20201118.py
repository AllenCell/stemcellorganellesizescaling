...
for i in np.arange(len(growvec)):
    ...
    growmax = growvec[i, 2]
    struct = ann_st[i, 2]
    axGrowVarS.plot(panAll[i,0],growval,'.',markersize=10, color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
    axGrowVarS.plot([panAll[i, 1], panAll[i, 2]], [growval, growval], color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor) , alpha=alpha,linewidth=lw_scatter)
    axGrowVarS.plot([panAll[i, 0], panAll[i, 0]], [growmin, growmax], color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor), alpha=alpha,linewidth=lw_scatter)
...
for i in np.arange(len(growvec)):
    ...
    struct = ann_st[i, 1]
    panxval = panx[i]
    axGrowVarSB.plot(panAll[i,0],0,'.',markersize=10,color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
    axGrowVarSB.plot([panAll[i,0], panxval], [0, 1], color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
...
plot_save_path = pic_root / f"heatmap_v9_20201118_res1000_v2.png"
plt.savefig(plot_save_path, format="png", dpi=1000)
plot_save_path = pic_root / f"heatmap_v9_20201118_res600_v2.png"
plt.savefig(plot_save_path, format="png", dpi=600)
plot_save_path = pic_root / f"heatmap_v9_20201118_res300_v2.png"
plt.savefig(plot_save_path, format="png", dpi=300)
plot_save_path = pic_root / f"heatmap_v9_20201118_v2.svg"
