...
CMT = np.zeros((len(structures['Structure'])+len(FS["cellnuc_metrics"]),len(FS["pca_components"])))
...
xlim = axNuc.get_xlim()
ylim = axNuc.get_ylim()
axNuc.text(xlim[0]-0.06*(xlim[1]-xlim[0]),ylim[1],'A',fontsize=fsP,fontweight='bold',va='top')
...
xlim = axLin.get_xlim()
ylim = axLin.get_ylim()
axLin.text(xlim[0]-0.15*(xlim[1]-xlim[0]),ylim[1],'B',fontsize=fsP,fontweight='bold',va='top')
...
for i in range(CMT.shape[0]):
    for j in range(CMT.shape[1]):
        val = CMT[i, j]
        if abs(val) > .7:
            text = axScale.text(j, i, np.round(CMT[i, j],2),
...
axScale.text(xlim[0]-0.5*(xlim[1]-xlim[0]),ylim[1],'C',fontsize=fsP,fontweight='bold',va='top')
...
# plot_save_path = pic_rootT / f"sizescaling_supfig_v3_20201202_res300.png"
# plt.savefig(plot_save_path, format="png", dpi=300)
# plot_save_path = pic_rootT / f"sizescaling_supfig_v3_20201202.svg"
