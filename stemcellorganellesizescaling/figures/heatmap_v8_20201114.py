...
ScaleMat = pd.read_csv(data_root / 'growing' / "ScaleStats_20201113.csv")
print(np.any(cells_COMP.isnull()))
...
axScale6.text(0.5,0.5,'FOR SALE',horizontalalignment='center')
...
oscatter(
    ...
    'LAMP1',
    HM['cellnuc_heatmap_abbs'][0],
    f"{structures.loc[structures['Gene']=='LAMP1','Structure'].values[0]} vol",
    'Structure volume',
    cells,
    ps,
    kde_flag=True,
...
for i in np.arange(len(growvec)):
    ...
    struct = ann_st[i, 1]
    growyval = growy[i]
    axGrowVarSS.plot(0,growval,'.',markersize=10,color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
    axGrowVarSS.plot([0, 10], [growval, growyval], color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
    axGrowVarSS.text(10,growyval, struct, fontsize=fs, color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor), verticalalignment='center',fontweight='normal',horizontalalignment='left',fontname=fn)
...
for i in np.arange(len(growvec)):
    ...
    growmax = growvec[i, 2]
    struct = ann_st[i, 2]
    axGrowVarS.plot(pan[i,0],growval,'.',markersize=10, color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
    axGrowVarS.plot([pan_min[i, 0], pan_max[i, 0]], [growval, growval], color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor) , alpha=alpha,linewidth=lw_scatter)
    axGrowVarS.plot([pan[i, 0], pan[i, 0]], [growmin, growmax], color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor), alpha=alpha,linewidth=lw_scatter)
...
for i in np.arange(len(growvec)):
    ...
    struct = ann_st[i, 1]
    panxval = panx[i]
    axGrowVarSB.plot(pan[i,0],0,'.',markersize=10,color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
    axGrowVarSB.plot([pan[i,0], panxval], [0, 1], color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor))
    axGrowVarSB.text(panxval, 1.1, struct, fontsize=fs, fontname=fn, color=adjust_lightness(color_st.loc[i, 'Color'],amount=lightnessfactor), verticalalignment='bottom',fontweight='normal',horizontalalignment='center',rotation=90)
axGrowVarSB.set_ylim(bottom=0,top=5)
axGrowVarSB.set_xlim(left=0, right=75*(1*(x8s+x8)/x8))
axGrowVarSB.axis("off")



plot_save_path = pic_root / f"heatmap_v8_20201117_res1000.png"
plt.savefig(plot_save_path, format="png", dpi=1000)
plot_save_path = pic_root / f"heatmap_v8_20201117_res600.png"
plt.savefig(plot_save_path, format="png", dpi=600)
plot_save_path = pic_root / f"heatmap_v8_20201117_res300.png"
