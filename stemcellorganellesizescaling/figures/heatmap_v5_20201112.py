...
for xi, xlabel in enumerate(
    ['cell_A', 'cell_V', 'cell_AV', 'nuc_A', 'nuc_V', 'nuc_AV']):
    for yi, ylabel in enumerate(FS['struct_metrics']):
        selected_structures = cells["structure_name"].unique()
        StructGrow[f"{xlabel}_{ylabel}"] = np.nan
        for si, struct in enumerate(selected_structures):
...
keepcolumns.append(f"nuc_metrics_{comp_type}_{struct_metric}")
for xi, xlabel in enumerate(HM['cellnuc_heatmap_COMP_metrics'][0:2]):
    keepcolumns.append(f"{xlabel}_{struct_metric}_COMP_{lin_type}_nuc_metrics_{comp_type}")
for xi, xlabel in enumerate(HM['cellnuc_heatmap_COMP_metrics'][2:4]):
    keepcolumns.append(f"{xlabel}_{struct_metric}_COMP_{lin_type}_cell_metrics_{comp_type}")
keepcolumns.append(f"cell_metrics_{comp_type}_{struct_metric}")
...
panCompC = panComp[:,[0, 1, 2]]
panCompN = panComp[:,[3, 4, 5]]
plot_arrayCompC = plot_arrayComp.iloc[:,[0,1,2]]
plot_arrayCompN = plot_arrayComp.iloc[:,[3,4,5]]
panCompC_L = np.expand_dims(panCompC[:,0],axis=1)
panCompC_R = panCompC[:,1:]
plot_arrayCompC_L = plot_arrayCompC.iloc[:,0]
plot_arrayCompC_R = plot_arrayCompC.iloc[:,1:]
panCompN_L = panCompN[:,0:-1]
panCompN_R = np.expand_dims(panCompN[:,-1],axis=1)
plot_arrayCompN_L = plot_arrayCompN.iloc[:,0:-1]
plot_arrayCompN_R = plot_arrayCompN.iloc[:,-1]
...
# Cell Comp rates
axCompVarC_L= fig.add_axes([w6+x4+w7+x5+w8+x6+w9, yh+h3, x7/3, y4])
axCompVarC_L.imshow(panCompC_L, aspect='auto', cmap='RdBu_r',vmin=-100, vmax=100)
for i in range(len(plot_arrayCompC_L)):
    for j in range(1):
        val = np.int(panCompC_L[i, j])
        text = axCompVarC_L.text(j, i, val,
                          ha="center", va="center", color="w", fontsize=fs_num, fontweight='bold', fontname = fn)
axCompVarC_L.set_yticks([])
axCompVarC_L.set_yticklabels([])
xlabels = ['Nuc v+a']
axCompVarC_L.set_xticks([])
for j, xlabel in enumerate(xlabels):
    xls = xlabel.split( )
    axCompVarC_L.text(j,1.03*len(plot_arrayCompC)-.5,xls[0],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
    axCompVarC_L.text(j,1.06*len(plot_arrayCompC)-.5,xls[1],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
axCompVarC_R= fig.add_axes([w6+x4+w7+x5+w8+x6+w9+x7/3, yh+h3, 2/3*x7, y4])
axCompVarC_R.imshow(panCompC_R, aspect='auto', cmap='PRGn',vmin=-50, vmax=50)
for i in range(len(plot_arrayCompC_R)):
    for j in range(len(plot_arrayCompC_R.columns)):
        val = np.int(panCompC_R[i, j])
        text = axCompVarC_R.text(j, i, val,
                          ha="center", va="center", color="w", fontsize=fs_num, fontweight='bold', fontname = fn)
axCompVarC_R.set_yticks([])
axCompVarC_R.set_yticklabels([])
xlabels = HM["cellnuc_COMP_abbs"][0:2]
axCompVarC_R.set_xticks([])
for j, xlabel in enumerate(xlabels):
    xls = xlabel.split( )
    axCompVarC_R.text(j,1.03*len(plot_arrayCompC)-.5,xls[0],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
    axCompVarC_R.text(j,1.06*len(plot_arrayCompC)-.5,xls[1],horizontalalignment='center',verticalalignment = 'center', fontname = fn)

# Nuc Comp rates
axCompVarN_L= fig.add_axes([w6+x4+w7+x5+w8+x6+w9+x7+w12, yh+h3, 2/3*x7, y4])
axCompVarN_L.imshow(panCompN_L, aspect='auto', cmap='PRGn',vmin=-50, vmax=50)
for i in range(len(plot_arrayCompN_L)):
    for j in range(len(plot_arrayCompN_L.columns)):
        val = np.int(panCompN_L[i, j])
        text = axCompVarN_L.text(j, i, val,
                          ha="center", va="center", color="w", fontsize=fs_num, fontweight='bold', fontname = fn)
axCompVarN_L.set_yticks([])
axCompVarN_L.set_yticklabels([])
xlabels = HM["cellnuc_COMP_abbs"][2:4]
axCompVarN_L.set_xticks([])
for j, xlabel in enumerate(xlabels):
    xls = xlabel.split( )
    axCompVarN_L.text(j,1.03*len(plot_arrayCompN_L)-.5,xls[0],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
    axCompVarN_L.text(j,1.06*len(plot_arrayCompN_L)-.5,xls[1],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
axCompVarN_R= fig.add_axes([w6+x4+w7+x5+w8+x6+w9+x7+w12+2/3*x7, yh+h3, 1/3*x7, y4])
axCompVarN_R.imshow(panCompN_R, aspect='auto', cmap='RdBu_r',vmin=-100, vmax=100)
for i in range(len(plot_arrayCompN_R)):
    for j in range(1):
        val = np.int(panCompN_R[i, j])
        text = axCompVarN_R.text(j, i, val,
                          ha="center", va="center", color="w", fontsize=fs_num, fontweight='bold', fontname = fn)
axCompVarN_R.set_yticks([])
axCompVarN_R.set_yticklabels([])
xlabels = ['Cell v+a']
axCompVarN_R.set_xticks([])
for j, xlabel in enumerate(xlabels):
    xls = xlabel.split( )
    axCompVarN_R.text(j,1.03*len(plot_arrayCompN_R)-.5,xls[0],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
    axCompVarN_R.text(j,1.06*len(plot_arrayCompN_R)-.5,xls[1],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
