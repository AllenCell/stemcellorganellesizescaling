...
Grow = pd.read_csv(data_root / 'growing' / "Growthstats_20201030.csv")
print(np.any(cells_COMP.isnull()))
...
axGrowB.plot(xbincenter[[start_bin, start_bin]], ylim, '--', linewidth=lw,color='red')
axGrowB.plot(xbincenter[[end_bin, end_bin]], ylim, '--', linewidth=lw,color='red')
...
axTransition.plot([xlim[0], xbincenter[start_bin]], [ylm, 0], '--', linewidth=lw,color='darkred')
axTransition.plot([np.mean(xlim), xbincenter[end_bin]], [ylm, 0], '--', linewidth=lw,color='darkred')
axTransition.plot([np.mean(xlim), xbincenter[start_bin]], [ylm, 0], '--', linewidth=lw,color='orangered')
axTransition.plot([xlim[1], xbincenter[end_bin]], [ylm, 0], '--', linewidth=lw, color='orangered')
...
#     axScale3B,
#     axScale3S,
#     HM["cellnuc_heatmap_COMP_metrics"][3],
#     'NPM1',
#     HM["cellnuc_COMP_abbs"][3],
#     'NPM1 *C',
#     'Structure volume',
#     HM["COMP_type"],
#     HM["LIN_type"],
...
axOrgGrow.text(0,1.03*len(growvec)-.5,'Growth',horizontalalignment='center',verticalalignment = 'center', fontname = fn)
...
axCompVar.set_xticks([])
for j, xlabel in enumerate(xlabels):
    xls = xlabel.split( )
    axCompVar.text(j,1.03*len(plot_arrayComp)-.5,xls[0],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
    axCompVar.text(j,1.06*len(plot_arrayComp)-.5,xls[1],horizontalalignment='center',verticalalignment = 'center', fontname = fn)
# axCompVar.set_title('Residual variance',verticalalignment='top')
...
axGrowVarS.set_xlabel('Structure volume explained by cell volume (%)')
axGrowVarS.set_ylabel('Structure growth when cell volume doubles  (%)')
axGrowVarS.grid()
...
axExpVarBar.set_xticklabels(['Neg. correlation', 'Pos. correlation'],verticalalignment='center')
axExpVarBar.set_title('Explained Variance (%)',verticalalignment='top')
...
axGrowBar.set_title('Growth rate (%)',verticalalignment='top')
