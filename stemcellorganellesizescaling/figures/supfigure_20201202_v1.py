...
# axNuc.legend([f"All cells (n={len(cells)})",f"Cells with sperical nuclei (n={len(xs)})",'Perfect spheres'])
#
# #% Fitting
# xL = xs.copy()
...
# axNuc.plot(xs,yL,'r.',markersize=1)
#
# xC = xs.copy()
# xC = xC**(2/3)
...
# axNuc.plot(xs,yC,'r.',markersize=1)
...
for i, typ in enumerate(CompMat['Type'].unique()):
    ...
    for j, ord in enumerate(order):
        ylevel = ylevel + 1
        y = [ylevel + yfac, ylevel + yfac, ylevel + 1 - yfac, ylevel + 1 - yfac]
        #lin
        x = MatLin[ord,[1,2,2,1]]
        axLin.fill(x,y,fc=LinPatchColor,ec='None')
        xp = MatLin[ord,0]
        axLin.plot(xp, ylevel+.5, 'o',mfc=LinPointColor, mec='None',markersize=ms)
        #com
        x = MatCom[ord, [1, 2, 2, 1]]
        axLin.fill(x, y, fc=ComPatchColor, ec='None')
        xp = MatCom[ord, 0]
        axLin.plot(xp, ylevel + .5, 'o', mfc=ComPointColor, mec='None', markersize=ms)
axLin.set_xlim(left=-5,right=100)
axLin.set_ylim(bottom=0,top=ylevel+1)
