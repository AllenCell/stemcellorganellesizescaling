...
#%% Data preparation
dataset = 'SizeScaling_20200828.csv'
...
for yi, metric in enumerate(selected_metrics):
    for xi, struct in enumerate(selected_structures):
        ...
        if xw==0:
            if yi == 0:
                plt.text(np.mean(xlim), ylim[0]+1.3*(ylim[1]-ylim[0]), struct, fontsize=fs2, horizontalalignment='center')
                plt.text(np.mean(xlim), ylim[0] + 1.2 * (ylim[1] - ylim[0]), selected_structures_org[xi], fontsize=fs2,
                         horizontalalignment='center')
                plt.text(np.mean(xlim), ylim[0] + 1.1 * (ylim[1] - ylim[0]), f"n= {len(x)}", fontsize=fs2,
                         horizontalalignment='center')
            if xi == 0:
                plt.figtext(0.5, h1 + ((row - 1) * (yw + yy + h2)) - h2/2, metric, fontsize=fs3, horizontalalignment='center')
        else:
